...
### Fixed
- **MCQ Session Storage Leak**: MCQ selection, shuffled order, and question text are now properly cleared from `sessionStorage` upon engine destruction. This ensures that when studying basic cards and returning to an MCQ card later, the card starts as a blank slate.
